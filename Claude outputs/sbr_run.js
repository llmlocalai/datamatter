...
async function anomalies(client, loadId, closedYears) {
  ...
  const { rows } = await client.query(`
    ...
    full AS (
      SELECT treasury_account FROM obs WHERE ratio IS NOT NULL
       GROUP BY treasury_account HAVING count(*) = $4),
    stat AS (
      SELECT o.treasury_account,
             percentile_cont(0.5) WITHIN GROUP (ORDER BY o.ratio) AS med
        FROM obs o JOIN full f USING (treasury_account)
       WHERE o.ratio IS NOT NULL GROUP BY o.treasury_account),
    dev AS (
      SELECT o.treasury_account, o.fiscal_year, o.ratio, s.med,
             percentile_cont(0.5) WITHIN GROUP (ORDER BY abs(o.ratio - s.med))
               OVER (PARTITION BY o.treasury_account) AS mad
        FROM obs o JOIN stat s USING (treasury_account) WHERE o.ratio IS NOT NULL)
    SELECT treasury_account, fiscal_year, ratio, med, mad,
           LEAST(99, abs(0.6745 * (ratio - med) / GREATEST(mad, abs(med) * 0.05, 1e-6))) AS z
      FROM dev`, [loadId, SCOPE, closedYears, closedYears.length]);
  const m = new Map();
...
async function buildCases(client, loadId, years, closedYears) {
  ...
  for (const c of cases) {
    ...
    const sev = (await client.query(
      `SELECT severity FROM dm_sbr_test WHERE code = ANY($1)`, [codes])).rows
      .map((r) => r.severity);
    const worst = sev.sort((a, b) => SEVERITY_RANK[b] - SEVERITY_RANK[a])[0] ?? 'moderate';

    const acct = (await client.query(
      `SELECT treasury_account_name, federal_account, agency_code, obligations_incurred,
              total_budgetary_resources
         FROM dm_exec_resource
        WHERE load_id = $1 AND scope = $2 AND fiscal_year = $3 AND treasury_account = $4`,
      [loadId, SCOPE, c.fiscal_year, c.treasury_account])).rows[0] ?? {};
    ...
    for (const f of factors) {
      await client.query(
        `INSERT INTO dm_sbr_case_factor (load_id, case_key, factor, points, max_points,
           detail, sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [loadId, c.case_key, f.factor, f.points.toFixed(2), f.max, f.detail, f.sort]);
    }
    n += 1;
  }
  return n;
