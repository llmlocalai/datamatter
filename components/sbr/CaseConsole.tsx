...
export default function CaseConsole({ caseKey, states, currentState }: {
  ...
  // The page shell is revalidated on a short window, so after a write the
  // record is re-read straight from the API rather than leaving a person
  // looking at their own change not having happened.
  async function refresh() {
    try {
      const r = await fetch(`/api/sbr?action=case&case=${encodeURIComponent(caseKey)}`,
        { cache: 'no-store' });
      const d = await r.json();
      if (r.ok) setLive(d);
    } catch { /* the page still shows the prerendered record */ }
  }
  ...
  async function run(code: string, record: boolean) {
    ...
    try {
      ...
      setResult(d);
      if (record) { setNotice('Recorded on the case timeline.'); await refresh(); }
    } catch (e: any) {
      ...
      {live?.case && (
        <div className="glass-card rounded-lg p-5">
          ...
          <p className="text-sm text-navy-200 mt-2">
            State <span className="font-semibold">{String(live.case.state ?? 'open').replace(/_/g, ' ')}</span>
            {live.case.ownerOrg ? `, owner ${live.case.ownerOrg}` : ''}
            {live.case.dueDate ? `, due ${String(live.case.dueDate).slice(0, 10)}` : ''}.
            {' '}{(live.events?.length ?? 0)} entr{(live.events?.length ?? 0) === 1 ? 'y' : 'ies'} on the timeline.
          </p>
          <p className="text-[11px] text-navy-500 mt-1.5">
            Read from the database just now. The record further down the page is from the
            prerendered shell and catches up within its revalidation window.
          </p>
        </div>
      )}
