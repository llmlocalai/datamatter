...
/**
 * The execution view: contract obligations for one FPDS acquisition program.
 * Reached from a budget line, or directly at /program?code=<code>, which is the
 * URL this page has always had and which still works.
 */
export default async function ExecutionView({ searchParams }:
  ...
  const [recipients, competed, pricing, psc, offices, awards, accounts, budgetLines] = await Promise.all([
    ...
    getLinesForProgramCode(program.programCode),
  ]);
    ...
    <Shell>
      ...
      <div className="mt-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Link href="/program" className="text-sm text-accent-400 hover:underline">← the budget-line roster</Link>
          <span className="text-navy-600">·</span>
          <span className="text-sm text-navy-400">
            this is the execution view for one FPDS acquisition program code
          </span>
        </div>
      ...
      {/* ------------------------------------------------ back to the budget */}
      <Section title="The budget lines this program code reaches"
        note="The other end of the chain. The President's Budget -1 exhibits name budget lines; FPDS names acquisition program codes; neither carries the other's key, so each row here states the evidence the link rests on.">
        {budgetLines.length === 0 ? (
          <p className="text-sm text-navy-400">
            No budget line in the President&rsquo;s Budget books held here shares a type designator or
            an identical name with <span className="font-mono">{program.programName}</span>. Nothing
            weaker than that is accepted (control{' '}
            <Link href="/controls" className="text-accent-400 hover:underline">EXH-09</Link>), so this
            is a gap in the crosswalk rather than a statement that no such line exists.
          </p>
        ) : (
          <DataTable
            caption="A link means the two names refer to the same system. It does not assert that these lines' appropriations funded the contracts above — an obligation draws on balances appropriated across several prior years, and the contract file states no such attribution."
            head={['Budget line', 'Exhibit', 'Treasury account', 'Line item', 'Linked on']}
            rows={budgetLines.map((b) => [
              <Link key={`${b.exhibit}${b.account}${b.bli}`}
                href={`/program?bli=${encodeURIComponent(`${b.exhibit}:${b.account}:${b.bli}`)}`}
                className="text-accent-400 hover:underline">{b.bliTitle}</Link>,
              b.exhibit.toUpperCase(),
              <span key={`${b.exhibit}${b.account}${b.bli}-a`} className="font-mono text-[12px]">
                {b.treasuryAccount ?? b.account}
              </span>,
              <span key={`${b.exhibit}${b.account}${b.bli}-b`} className="font-mono text-[12px]">{b.bli}</span>,
              b.matchMethod === 'exact_name' ? 'identical names' : `type designator ${b.matchEvidence}`,
            ])} />
        )}
      </Section>
